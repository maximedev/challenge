(function () {
  ...
  /* @ngInject */
  function triMenuItemController($scope, $mdSidenav, $state, $filter) {
    ...
    var toggleDropDownListener, openParentsListener, stateChangeSuccessListener;
    switch (triMenuItem.item.type) {
      case 'dropdown':
        ...
        toggleDropDownListener = $scope.$on('toggleDropdownMenu', function (event, item, open) {
        ...
        openParentsListener = $scope.$on('openParents', function () {
      ...
      case 'link':
        ...
        stateChangeSuccessListener = $scope.$on('$stateChangeSuccess', function (event, toState, toParams) {
          checkItemActive(toState.name, toParams);
        });
        break;
    }

    $scope.$on('$destroy', toggleDropDownListener);
    $scope.$on('$destroy', openParentsListener);
    ...
    function toggleDropdownMenu() {
      $scope.$parent.$broadcast('toggleDropdownMenu', triMenuItem.item, !triMenuItem.item.open);
    }
